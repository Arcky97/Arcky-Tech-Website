...
<Callout type="hint">
  **Note:** The only two options <Rd>not supported</Rd> for `pbShelfMart` (Shelf events) are `useCat` and `cantSell`. All other options are shared between Shelf and Mart events. 
</Callout>